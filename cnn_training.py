...
def read_dataset(path):
    ...
    for pa in my_list:
        i=i+1
        print(pa,"==================")
        for root, dirs, files in os.walk(r'C:\Users\Amishazzz\Desktop\final back\fake_currency_detection\fake_currency_detection\Dataset\Training\\' + pa):

         for f in files:
            file_path = os.path.join(r'C:\Users\Amishazzz\Desktop\final back\fake_currency_detection\fake_currency_detection\Dataset\Training\\'+pa, f)
...
if not os.path.exists("Final_model.h5"):
    ...
    model.save("Final_model.h5")  # train for randomly selected one
else:
    model = load_model("Final_model.h5")  # load weights
